...
def get_action_sample_forbid_multi(args):
    """
    sample with probability
    :param pro_list:
    :param length:
    ...
    for action in forbidden_actions:
        pro_list[action] = 0
    action = torch.multinomial(pro_list, 1)
    return action.item()
...
def get_action_max_forbid_multi(args):
    """
    choose the action with the largest probability
    :param pro_list:
    :param length:
...
class PPO_Log(nn.Module):
    def __init__(self, BackBone=BackboneNet, Actor=ActorNet, Critic=CriticNet,
        ...
        self.critic = Critic(criticParam.in_size, criticParam.out_size, criticParam.hide_size_list, criticParam.hide_size_fc,
                             ...
                             criticParam.concat)
        self.actor = Actor(actorParam.in_size, actorParam.out_size, actorParam.hide_size_list, actorParam.hide_size_fc, actorParam.n_head_list,
                                  actorParam.n_layers, actorParam.concat)
    ...
    def forward(self, data_batch, max_size):
        ...
        # 计算动作
        action_pro = self.actor(feature, edge_index, max_size, edge_weight)

        return value, action_pro

    def forward_log(self, data_batch, max_size, actions):
        data_batch_ = data_batch.clone().to(device)
        x = Variable(data_batch_.x.float().to(device))
        edge_index = Variable(data_batch_.edge_index.to(device))
        edge_attr = Variable(data_batch_.edge_attr.to(device))
        ...
        # action
        action_probs = self.actor(feature, edge_index, max_size, edge_weight)
    ...
    def work_forbid(self, data_batch, len_list, max_size, forbidden_actions_list, type_='selectAction'):
        """
        产生动作
        ...
        pool = pathos_mp.ProcessingPool()
        args_list = zip(action_prob_list, len_list, forbidden_actions_list)
        if type_ == 'selectAction':
            actions = pool.map(get_action_sample_forbid, action_prob_list, len_list, forbidden_actions_list)
            # actions = pool.map(get_action_sample_forbid_multi, args_list)
            # pool.close()
            # pool.join()
            actions = torch.tensor(list(actions), dtype=torch.long).view(-1, 1)
            actions_prob = action_prob.gather(dim=1, index=actions).to(device)
            actions = actions.to(device)
        if type_ == 'selectActionMax':
            # actions = pool.map(get_action_max_forbid, action_prob_list, len_list, forbidden_actions_list)
            actions = pool.map(get_action_max_forbid_multi, args_list)
            # pool.close()
            # pool.join()
            actions = torch.tensor(list(actions), dtype=torch.long).view(-1, 1).to(device)
    ...
    def trainStep(self, ep, max_size, batchSize=None):
        ...
        old_action_log_prob = torch.tensor(old_action_log_prob, dtype=torch.float).view(-1, 1).to(device)
        ...
        for i in range(1, self.K_epoch + 1):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(len(graphs))), batchSize, False):
                ...
                # 计算优势
                V, probs = self.forward(graphs_index, max_size)
                delta = Gt_index.view(-1, 1) - V
                advantage = delta.view(-1, 1).detach()

                # 获取动作概率
                actions_prob = probs.gather(1, actions[index])

                ratio = (actions_prob / old_action_log_prob[index])
                surr1 = ratio * advantage
                surr2 = clamp(ratio, 1 - self.eps_clips, 1 + self.eps_clips) * advantage

                loss_a = -torch.min(surr1, surr2).mean()
                loss_c = F.mse_loss(Gt_index.view(-1,1), V)
    ...
    def trainStep_log(self, ep, max_size, batchSize=None):
        """
        模型训练
        :param ep: 当前的轮次
        :param batchSize: 训练的batch
