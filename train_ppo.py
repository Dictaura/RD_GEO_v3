...
def main():
    ...
    env = RNA_Graphs_Env(dataset, cal_freq=cal_freq_start, max_size=max_size)
    ...
    lr_actor = 0.000000001  # learning rate for actor network
    lr_critic = 0.000000001  # learning rate for critic network
    ...
    agent = PPO(BackboneNet, ActorNet, CriticNet,
        backboneParam, criticParam, actorParam, lr_critic, lr_actor, train_batch_size=batch_size,
                K_epoch=K_epochs, eps_clips=eps_clip, actor_freeze_ep=actor_freeze_ep, num_graph=len(env.len_list), gamma=gamma).to(device)
    ...
    while time_step < max_train_timestep:
        ...
        for t in range(1, max_ep_len+1):
            ...
            # 智能体产生动作
            actions, probs = agent.work_forbid(state_, env.len_list, max_size, env.forbidden_actions_list, type_=action_type)

            # 环境执行动作
            next_state, reward_list, is_termial, done_list, ids = env.step(actions, t, reward_type=reward_type,
                                                                           done_type=done_type)
            ...
            if t == max_ep_len:
                pass
            for graph, action, prob, reward, next_graph, id, done in zip(state_.clone().to_data_list(), actions, probs,
        ...
        final_energy = pool.map(get_energy_base, final_seqs_base, final_dotBs)
        final_energy = list(final_energy)
        final_distance = pool.map(get_distance_from_base, final_seqs_base, final_dotBs)
